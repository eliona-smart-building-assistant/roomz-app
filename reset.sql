...
DROP SCHEMA IF EXISTS roomz CASCADE;

DELETE FROM heap
WHERE asset_id IN (
	SELECT asset_id
	FROM asset
	WHERE asset_type LIKE E'roomz\\_%'
);
...
WHERE widget_id IN (
	...
	WHERE public.dashboard.name LIKE 'ROOMZ%'
);
...
WHERE dashboard_id IN (
	...
	WHERE name LIKE 'ROOMZ%'
);

DELETE FROM public.dashboard
WHERE name LIKE 'ROOMZ%';

-- DELETE FROM eliona_app WHERE app_name = 'roomz';
-- DELETE FROM eliona_store WHERE app_name = 'roomz';
